...
function chunkByParagraphs(text: string, maxParagraphsPerPage = 10) {
  ...
  for (let i = 0; i < rawParagraphs.length; i++) {
    ...
    if (currentItems.length >= maxParagraphsPerPage || (currentLength + p.length > 2100 && currentItems.length > 0)) {
      pages.push({ chapterTitle: currentChapterTitle, items: currentItems });
      currentItems = [];
      currentLength = 0;
    }
