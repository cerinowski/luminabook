...
import { Sparkles, Zap, Upload, Loader2, FileDown, BookOpen, ChevronLeft, CreditCard, LayoutDashboard, Palette, Type, MessageSquare, Download, CheckCircle2, RefreshCw } from 'lucide-react';
    ...
    return new Promise((resolve) => {
        ...
        const renderText = (loadedImg: HTMLImageElement | null) => {
            if (loadedImg) {
                ...
                // Usamos as cores do tema para o texto se a imagem falhar
...
export default function Home() {
    ...
    const handleGenerateVariations = async () => {
        ...
        addLog(`Iniciando Geração Incremental (Nano-Banana G8)...`);
        ...
        try {
            ...
            // GERAÇÃO SEQUENCIAL PARA EVITAR 504 E DAR FEEDBACK IMEDIATO
            for (let i = 0; i < 4; i++) {
                addLog(`Preparando Variação ${i + 1}...`);
                const artPrompt = `${theme.image_generation_prompt}. The book title "${title.toUpperCase()}" is written in beautiful, large 3D typography at the center. The author name "${author.toUpperCase()}". High-end 8k photography.`;
                ...
                try {
                    ...
                    if (data.base64 && !data.error) {
                        addLog(`${data.engine || 'Nano'} V${i + 1} OK`);
                        finalImage = data.base64;
                        setBaseArtUrls(p => { const n = [...p]; n[i] = data.base64; return n; });
                    } else {
                        throw new Error(data.error || "Engine Timeout");
                    }

                    setVariations(p => { const n = [...p]; n[i] = finalImage; return n; });
                    setVariationsStatus(p => { const n = [...p]; n[i] = 'done'; return n; });
                } catch (e) {
                    addLog(`V${i + 1} Fallback Gradiente`);
                    const fall = await generateTypographyLayer(null, { title, author, primary: theme.primary_color, secondary: theme.secondary_color, font: selectedFont });
                    setVariations(p => { const n = [...p]; n[i] = fall; return n; });
                    setVariationsStatus(p => { const n = [...p]; n[i] = 'fail'; return n; });
                }
            }
        } catch (e) {
            addLog("Erro Crítico no Fluxo de Geração.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <section className="flex-1 flex flex-col relative overflow-hidden">
                <header className="h-20 border-b border-white/5 flex items-center justify-between px-10 bg-[#050510]/80 backdrop-blur-xl z-20">
                    ...
                    {selectedCover && activeTab !== 'export' && (
                        <button onClick={handleCreateEbook} className="bg-gradient-to-r from-purple-600 to-pink-500 px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest hover:shadow-lg hover:shadow-purple-500/30 transition-all shadow-xl">Prosseguir</button>
                    )}
                </header>
                    ...
                    <AnimatePresence mode="wait">
                        {activeTab === 'config' && (
                            <motion.div key="config" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-xl space-y-8">
                                <h3 className="text-4xl font-black tracking-tighter">Motor Nano-Banana G8</h3>
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-white/30">Título</label>
                                        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Ex: O Poder da Mente" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-lg font-bold outline-none focus:border-purple-500" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-white/30">Autor</label>
                                        <input value={author} onChange={e => setAuthor(e.target.value)} placeholder="Seu nome" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-lg font-bold outline-none focus:border-purple-500" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-white/30">Conceito Visual</label>
                                        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Minimalista, épico, flores..." rows={3} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-base font-medium outline-none focus:border-purple-500 resize-none" />
                                    </div>
                                    <button onClick={handleGenerateVariations} disabled={isLoading || !title} className="w-full py-5 bg-white text-black font-black uppercase tracking-widest rounded-2xl hover:bg-purple-200 transition-all flex items-center justify-center gap-3">
                                        {isLoading ? <Loader2 className="animate-spin" /> : <Zap className="w-5 h-5 fill-current" />} Pintar Capas G8
                                    </button>
                                </div>
                            </motion.div>
                        )}
                                        ...
                                        <div key={i} className={`relative aspect-[2/3] rounded-2xl overflow-hidden border-2 transition-all ${status === 'loading' ? 'bg-white/5 animate-pulse border-white/10 flex items-center justify-center' : (selectedCover === variations[i] ? 'border-purple-500 shadow-2xl' : 'border-white/5')}`}>
                                            {status === 'loading' ? (
                                                <div className="text-center space-y-2">
                                                    ...
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-white/20">Pintando V{i + 1}...</span>
                                                </div>
                                            ) : (
                                                <motion.img
                        ...
                        {activeTab === 'editorial' && (
                            <motion.div key="editorial" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col lg:flex-row gap-12">
                                <div className="flex-1 space-y-6">
                                    ...
                                    <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Cole o texto..." className="w-full bg-white/5 border border-white/10 rounded-3xl p-8 text-white/80 text-sm leading-relaxed min-h-[500px] outline-none focus:border-purple-500/50" />
                                </div>
                                <div className="w-full lg:w-[350px] sticky top-0 bg-white/5 p-6 rounded-3xl border border-white/10 space-y-6">
                                    ...
                                    <button onClick={() => setActiveTab('gallery')} className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest">Trocar Capa</button>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {activeTab === 'export' && (
                            <motion.div key="export" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center space-y-8">
                                ...
                                <h3 className="text-4xl font-black tracking-tighter">eBook Finalizado!</h3>
                                <button onClick={downloadPDF} className="bg-white text-black px-10 py-5 rounded-2xl font-black uppercase shadow-2xl flex items-center gap-3 hover:scale-105 transition-all"><FileDown className="w-5 h-5" /> Baixar PDF HD</button>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                ...
                {debugLogs.length > 0 && (
                    <div className="fixed bottom-6 right-6 w-80 bg-black/80 backdrop-blur-3xl border border-white/10 p-4 rounded-2xl flex-col gap-2 z-50 flex shadow-2xl">
                        <span className="text-[10px] font-black tracking-widest text-white/40 uppercase">Monitor Lumina G8</span>
